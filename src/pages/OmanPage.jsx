...
function OmanNavbar({ onPostAd }) {
  const [search, setSearch] = useState('')
  const [locationOpen, setLocationOpen] = useState(false)
  const [locationSearch, setLocationSearch] = useState('')
  const [catsOpen, setCatsOpen] = useState(false)
  const filteredCities = omanCities.filter(c => c.toLowerCase().includes(locationSearch.toLowerCase()))
  return (
      ...
      <div className="mx-auto max-w-[1232px] px-4 flex items-center gap-3 h-[60px]">
        ...
        <div className="relative shrink-0">
          <div onClick={() => { setLocationOpen(!locationOpen); setCatsOpen(false) }} className="flex items-center gap-1.5 border border-gray-300 rounded-md px-3 py-2 cursor-pointer hover:border-gray-400 transition h-[38px] select-none">
            ...
            <span className="text-[14px] text-gray-800 min-w-[46px]">Oman</span>
          ...
          {locationOpen && (<div className="absolute top-[44px] left-0 w-[280px] bg-white border border-gray-200 rounded-md shadow-xl z-50 overflow-hidden">
            ...
            <div className="px-4 py-2 cursor-pointer hover:bg-gray-50"><span className="text-[14px] font-medium text-[#DE1F26]">See ads in all Oman</span></div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <a href="/" className="text-[14px] font-medium text-gray-700 hover:text-[#DE1F26]">العربية</a>
...
function OmanHero(){return(<div className="w-full bg-white"><div className="mx-auto max-w-[1232px]"><img alt="Dubizzle Oman" className="w-full object-cover" src="https://olx-om-production.s3.eu-west-1.amazonaws.com/image/24379595/2a9bf0a0bd6d4a6691a19619c2fd8c95" style={{maxHeight:'200px'}}/></div></div>)}
...
const popularSearches=[{heading:'Properties For Sale',links:['Apartments in Muscat','Villas in Salalah','Land in Sohar','Penthouse in Muscat']},{heading:'Properties For Rent',links:['Flats in Muscat','Studios in Salalah','Offices in Sohar','Furnished Apartments']},{heading:'Cars For Sale',links:['Used Cars in Oman','Toyota in Oman','Nissan in Oman','BMW in Oman']},{heading:'Jobs',links:['IT Jobs','Engineering Jobs','Sales Jobs','Hospitality Jobs']}]
...
export default function OmanPage(){
  ...
  return(<div className="min-h-screen bg-[#F5F5F5] font-sans">
    <OmanNavbar onPostAd={()=>setModalOpen(true)}/>
    <OmanHero/><PopularCategories/>
